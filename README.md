...
| Rule | Purpose |
| --- | --- |
| `verified_files` | Admit private or closed inputs only after size and SHA-256 checks |
| `sentinel_test` | Byte-for-byte comparison of a generated file with a reference |
| `script_check` / `check_test` | Run a project's Python gate as a cached action and expose it as a test |
| `staged_file` | Copy an external-repository file into local outputs (Windows runfiles cannot reach `../repo` paths) |
| `image_splice` | Place regenerated byte ranges into a copy of a verified original, with provenance |
| `ghidra_pipeline` / `ghidra_oracle` / `ghidra_session` | Isolated headless Ghidra work and interactive sessions |

Closed, locally installed tool trees (historical compilers, SDKs) are exposed
with `local_tool_repository` from `@rules_decomp//repositories:local_tool.bzl`;
see [docs/checks.md](docs/checks.md) for the check, staging, splice, and local
tool APIs.
